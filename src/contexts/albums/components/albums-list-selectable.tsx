...
export default function AlbumsListSelectable({
  ...
  function handlePhotoOnAlbums(albumId: string) {
    ...
    updatePhotoAlbums(photo.id, albumsIds);
  }

  function updatePhotoAlbums(photoId: string, albumsIds: string[]) {
    setIsUpdatingPhoto(async () => {
      await managePhotoOnAlbum(photoId, albumsIds);
    });
  }
      ...
      {loading && (
        <>
          {Array.from({ length: 5 }).map((_, index) => (
            <Skeleton key={`album-loading-${index}`} className="h-[2.5rem]" />
          ))}
        </>
      )}
      {!loading && albums.length === 0 && <Text>Nenhum álbum encontrado</Text>}
    </ul>
  );
}
